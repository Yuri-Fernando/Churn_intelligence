...
# 8b. Processamento Distribuído + Streaming Assíncrono (V2)

Estende a Fase 1 (feature engineering) e a Fase 8 (streaming) com uma camada de execução
alternativa, preparada para volumes maiores e I/O real de produção. A V1 (pandas +
threading) permanece intacta; a V2 é aditiva.
...
(`dense_rank` via window function no lugar de `pd.factorize`), imputação de nulos com
mediana aproximada por partição e escrita em parquet particionado. Inclui
`feature_summary()`, uma agregação distribuída (`groupBy` + `avg`/`count`) de exemplo.
...
eventos sintéticos. `requirements_v2.txt` estende `requirements.txt` com `pyspark`.
